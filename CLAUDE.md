...
- Never remove: `.claude/` ignore entries (prettier/eslint/gitignore), `--cache` flags, or the `dist-types/` + `emitDeclarationOnly` setup in package tsconfigs. Keep `clean` preserving `dist-types/` while deleting `*.tsbuildinfo`; root `typecheck` stays incremental without `--force`.
