...
- A requirement may declare [Supported Application Envelope](SUPPORTED_APPLICATION_ENVELOPE.md) capabilities with backticked `` `capability:<slug>` `` markers inside its `FR`/`BR`/`NFR` item. Classification is deterministic and never inferred from prose: a known exclusion blocks intake and names the alternative, an unknown slug becomes a Blocking Question tied to the requirement identifier, and markers outside an identified requirement are Blocking Questions as well.
