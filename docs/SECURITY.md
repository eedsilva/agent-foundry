...
O adapter usa `read-only` para papéis não mutáveis e `workspace-write` para os demais, com aprovação desabilitada no modo não interativo.

**Leitura fora do workspace não é bloqueada pelo sandbox nativo do Codex.** `workspace-write` restringe apenas escrita (`writable_roots`); confirmado contra o próprio schema (`SandboxPolicy`/`SandboxWorkspaceWrite`) que não existe campo de restrição de leitura em nenhuma variante da política — diferente de Claude Code, não há um `--settings`-equivalente já embutido na CLI para ligar. Fechado por fora: o adapter agora embrulha o processo Codex inteiro (não só uma chamada de ferramenta) em `srt` (`@anthropic-ai/sandbox-runtime`, mesma primitiva Seatbelt/bubblewrap do sandbox do Claude Code, empacotada como wrapper de processo standalone) — `srt -d -s <settings> -- codex exec ...`, com `--sandbox workspace-write`/`read-only` mantido aninhado dentro (#637, ADR-0081). O arquivo de settings nega leitura da raiz de workspaces, do temp compartilhado do host e da mesma lista de credenciais do adapter Claude (`.ssh`, `.aws/credentials`, `.netrc`, `.docker/config.json`, `.npmrc`, `.git-credentials`, `.config/gh/hosts.yml`), reabrindo só o `cwd` do run e um temp efêmero próprio (mesma convenção `.agent-foundry-run-tmp` fora da worktree, mesma checagem de symlink por igualdade de `realpath()` do ADR-0071). `~/.codex` recebe escrita liberada explicitamente — medido, não suposto: uma chamada trivial do Codex escreve em sqlite de estado, cache de modelos, `installation_id`, locks de OAuth e chegou a baixar plugin de marketplace, então o carve-out real é "`auth.json` + escrita ampla em `~/.codex`", mais largo que uma capability de auth estritamente escopada.

**Rede: allowlist estática mínima, não política ADR-0076.** `network.allowedDomains` cobre só os endpoints de auth/inferência do provedor (`chatgpt.com`, `ab.chatgpt.com`, `api.openai.com`) e `registry.npmjs.org`, copiados literalmente da própria ADR-0076 — `github.com`/`developers.openai.com`, observados em tráfego real, ficam de fora (confirmado dispensável com reprodução: tarefa mutante completa com os dois bloqueados). **Loopback fica liberado por inteiro, não por porta** — `127.0.0.1`/`localhost` com `allowLocalBinding: true`, porque tarefas de forma DB tocam Postgres/Supabase local de verdade (reproduzido: sem essa liberação, a conexão falha com `EPERM`) e porque o sufixo `:porta` do `srt` foi medido como cosmético sob `allowLocalBinding` — um segundo listener em porta não allowlistada continuou alcançável. Isso é risco residual nomeado, não lacuna silenciosa: com loopback aberto, o processo Codex também alcança a API do control plane deste repositório, que ainda não tem sessão local autenticada (#597 fecha isso; #637 não é regressão, já que o Codex tinha rede irrestrita antes desta mudança).

**Isto não é conformidade com a ADR-0076.** Sem proxy DNS/HTTP auditado, sem origem de documentação pin­ada, sem loopback exato por porta, sem cobertura de instalação de dependência — declarado explicitamente como stopgap na ADR-0081, não uma implementação parcial disfarçada de completa. Ver ADR-0081 ("Non-conformance with ADR-0076") para a lista exaustiva de desvios e a decisão de escopo que os aceitou.

### Claude Code

O adapter usa `plan` para leitura e `acceptEdits` para mutação. `acceptEdits` só auto-aprova a família Edit/Write — Bash continua exigindo aprovação, e o modo headless (`-p`) não tem TTY para concedê-la, então todo comando shell seria negado por padrão (#537). Para runs mutantes, o adapter passa `--allowedTools` com uma allowlist restrita ao toolchain que os apps gerados por este repositório de fato usam: `pnpm`, `npm`, `npx`, `node`, `git`, `docker`, `supabase`, `psql`. Runs de leitura (`plan`) não recebem allowlist. Continua sem bypass global de permissões (`--dangerously-skip-permissions`) — comandos fora da allowlist, como entrypoint literal, permanecem negados.

**A allowlist não é uma fronteira contra injeção de comando — por isso o adapter também liga o sandbox nativo do Claude Code (#565, ADR-0071).** `Bash(<bin> *)` é um casamento de prefixo, não shell-aware: um comando como `git status && echo chained-ok` começa com `git ` e casa com `Bash(git *)`, executando o `&&`/`;`/`|`/`$()` encadeado sem checagem adicional (confirmado empiricamente contra a CLI real). As regras `Read`/`Edit` do Claude Code também não cobrem um subprocesso que lê arquivos por conta própria (`node -e "readFileSync(...)"` através de um binário permitido) — confirmado empiricamente como um caminho de exfiltração real antes desta mudança (ADR-0071). O adapter agora passa `--settings` com `sandbox.enabled: true`, confinando o Bash tool e seus subprocessos por SO (Seatbelt/bubblewrap) à raiz de workspaces e ao diretório temporário compartilhado do host (`denyRead`), reabrindo apenas o `cwd` do run e um diretório temporário efêmero próprio do run (`allowRead`) — path traversal, path absoluto e symlink apontando para fora do workspace são todos bloqueados nesse nível, verificado empiricamente. O temp por run fica fora da worktree de propósito: `TMPDIR`/`TEMP`/`TMP` do host chegam ao processo via `safeSpawnEnv`, e um temp dentro do `cwd` seria pego pelo `git add -A` que `FileWorkspaceManager` roda a cada checkpoint, entrando no histórico do app gerado (ver ADR-0071, "Third review round"). `docker` continua sem essa cobertura (o sandbox do Claude Code não o suporta; ver ADR-0071) e continua um risco aceito, não uma lacuna esquecida — em produção containerizada não há `docker.sock` montado, então a chamada já falha por conta própria; no fallback local confiável, o próprio operador já era a fronteira de confiança. `network.allowedDomains` fica em `["*"]` deliberadamente: #565 é sobre fronteira de filesystem, não política de rede.
