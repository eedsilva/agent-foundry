...
**A allowlist não é uma fronteira contra injeção de comando.** `Bash(<bin> *)` é um casamento de prefixo, não shell-aware: um comando como `git status && echo chained-ok` começa com `git ` e casa com `Bash(git *)`, executando o `&&`/`;`/`|`/`$()` encadeado sem checagem adicional (confirmado empiricamente contra a CLI real). Como o executor roda a CLI direto no host sem sandbox (ADR-0025), esse risco residual é aceito explicitamente, não uma lacuna esquecida: a allowlist limita quais binários um agente pode invocar como entrypoint, não o que uma invocação encadeada a partir de um binário permitido pode fazer. Ver ADR-0063.
