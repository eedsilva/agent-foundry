...
**Leitura fora do workspace não é bloqueada pelo sandbox nativo do Codex.** `workspace-write` restringe apenas escrita (`writable_roots`); confirmado contra o próprio schema (`SandboxPolicy`/`SandboxWorkspaceWrite`) que não existe campo de restrição de leitura em nenhuma variante da política. Um `Bash` que rode `node -e "readFileSync('<caminho fora do workspace>')"` ainda lê qualquer arquivo alcançável pelo processo. Esta é uma ameaça confirmada com reprodução (ver ADR-0071), aberta como issue de acompanhamento (#637) — fechar exige envolver o subprocesso do Codex num jail de SO próprio, fora do escopo de uma mudança de configuração.
