...
4. One fact is marked LOW CONFIDENCE / inferred where the docs are genuinely silent (see the "Process isolation model" callout in Section 2a); nothing else in this document rests on a secondary source, and no blog/forum material is used for a load-bearing claim (search-engine results are cited only as pointers to the official page, never as the source of a quote).
...
4. **Rate-limit-aware concurrency throttling (contingent).** Ticket 1's static concurrency cap draws from the same shared 5-hour/weekly pool as everything else on the account (Section 2d) — this spike didn't measure whether a conservative static cap is sufficient in practice. Once ticket 1 is running, evaluate whether bursts of parallel Codex tasks starve later sequential work in the same window; if they do, add rate-limit-window-aware backoff instead of (or in addition to) a static cap.
