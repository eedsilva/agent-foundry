...
Three tests, run back to back in one script execution on 2026-08-11 (times UTC, `codex-cli 0.146.1`):
...
4. **No native supervision/failure-propagation primitive from Codex itself.** Unlike Claude Code's background-subagent completion-notification and typed-API-error-failure model (Section 1), an orchestrator spawning raw `codex exec` processes must build its own process supervision, timeout, and result-collection layer — there is nothing in Codex CLI analogous to Claude's `Agent` tool for this.
