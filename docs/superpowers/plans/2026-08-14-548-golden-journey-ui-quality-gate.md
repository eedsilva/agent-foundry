...
**Goal:** Make the checked-in fake provider CLI answer the UI-quality judge's schema for real (it currently has no branch for it at all, so `evaluateUiQuality` always degrades to `undefined` against a real/subprocess executor), then prove — with a genuine subprocess spawn of that same fake CLI, not a hand-scripted mock — that a score at/above a configured threshold leaves a run's `approved` true and green, while a score below it flips `approved` false and triggers a repair.

**Architecture:** Two seams, in dependency order. (1) `packages/executors/src/fixtures/fake-cli/fake-cli-core.mjs` — the single shared core the real fake `claude`/`codex` CLI binaries AND `MockAgentExecutor` both delegate to — gets a new branch for `UI_QUALITY_JUDGE_JSON_SCHEMA`'s `$id`, returning a fixed, schema-conforming score. Proven at the executor boundary by extending `packages/executors/src/fake-cli.integration.test.ts` (the file that already round-trips the real `ClaudeCliExecutor`/`CodexCliExecutor` against these exact binaries). (2) `packages/composition/src/ui-quality-judge.integration.test.ts` already proves `gateOnUiQuality`'s both-directions behavior end-to-end (#477), but only against a hand-scripted mock class that special-cases the judge schema in-process — the mock exists specifically *because* the shared fixture had no judge branch (see that file's own comment on `UiQualityJudgeExecutor`, which this plan's Task 1 makes obsolete for this purpose, though the class stays for #477's own scripted-sequence needs). A new class in the same file wraps a **real** `ClaudeCliExecutor` for judge-schema requests only — routed through the checked-in fake CLI via a `PATH` prepend, mirroring `packages/composition/src/pipeline-regression.e2e.test.ts`'s established pattern — while every other step (task graph, implement, verify, the browser check itself) still runs through the proven `MockAgentExecutor` path. Two runs, two policy thresholds straddling the fixed fake score, prove both directions.
...
- [ ] Confirm no changes landed outside: `packages/executors/src/fixtures/fake-cli/fake-cli-core.mjs`, `packages/executors/src/fake-cli.integration.test.ts`, `packages/composition/src/ui-quality-judge.integration.test.ts` (`git diff --stat main...HEAD`).
