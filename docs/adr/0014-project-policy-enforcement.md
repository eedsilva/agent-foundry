...
- **Mid-run change:** at every step boundary the orchestrator re-resolves the policy and compares its hash against the run's pinned record; a mismatch emits `policy.violation` and fails the run. Resume of a paused run validates the current policy hash against the same pinned `run.policy` record and is blocked with a `policyVersion` diagnostic on drift, reusing the `ResumeBlockedError` machinery. ADR 0080 later narrowed retry: once candidate version history exists, Personal v1 refuses a fresh Task Agent run instead of binding it to an existing candidate or accepted stack. Application evolution is a later milestone.
...
- **Confirmation API to accept a mid-run policy change:** deferred. ADR 0080 prohibits using a fresh run against an existing candidate or accepted stack in Personal v1; an explicit application-evolution flow can add that fork later.
...
Validated by unit suites in contracts, model-router, executors, persistence, and orchestrator (provider block, forbidden package, disallowed command, hash stamping, mid-run change block + retry refusal after candidate history, resume block) plus the full `npm run check` gate.
