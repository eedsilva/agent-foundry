...
Claude `modelUsage` keys are treated as concrete executed-model identifiers. The parser aggregates those identifiers across all parsed provider documents before returning a value. More than one concrete identifier, conflicting explicit identifiers, or conflicting direct identifiers produces an unknown `executedModel`. Canary freezing must fail closed when the executed model is unknown.
