...
Add `packages/persistence/src/postgres/`: one adapter class per swapped port (`PostgresProjectRepository`, `PostgresWorkflowRunRepository`, `PostgresStepRunRepository`, `PostgresStepAttemptRepository`, `PostgresApprovalRequestRepository`, `PostgresApprovalDecisionRepository`, `PostgresEventStore`, `PostgresStepEventRepository`, `PostgresConversationRepository`, `PostgresArtifactStore`), all constructed from a shared `postgres.js` client (`createPostgresClient`, `PostgresDb`). The project mutation lock is the one exception (#602): it holds a transaction-scoped advisory lock for the whole critical section, so it gets a second, dedicated client. Sharing the store pool would deadlock — a section holds a connection while doing store I/O through the same pool, so `max` concurrent sections leave every connection held by a section waiting for one. `PostgresPreviewLifecycleLock` predates this and still runs on the store pool with the same hazard; issue #691 tracks it. Rows use a hybrid shape: columns for fields the domain queries or constrains (id, status, version, timestamps, foreign keys) and a `jsonb` column for the full validated entity, so reads stay a single round trip and writes stay schema-validated at the port boundary rather than duplicated across dozens of typed columns.

Prepared statements follow the target rather than a global switch: `createPostgresClient` keeps postgres.js's default (`prepare: true`) for direct Postgres and turns them off when the URL points at port 6543, the documented Supabase transaction pooler, which cannot serve named prepared statements. Detection is by port alone — every other query parameter postgres.js does not recognise is forwarded in the startup packet, where the server rejects it, so an invented flag would trade a broken query for a broken connection. Rollback is the same one-line predicate: dropping it restores the driver default everywhere, at the cost of the 6543 target. Operators pointing at a transaction pooler on another port pass the literal `?prepare=false` (`docs/OPERATIONS.md`); executed proof against a real pooler remains pending under #692 — what has been measured so far is PostgreSQL 17.10 directly, not Supavisor or PgBouncer.
