    ...
    identity.outputSchemaId === UI_QUALITY_JUDGE_SCHEMA_ID
      ? {
          ...
          criteria: UI_QUALITY_CRITERION_IDS.map((criterionId) => ({
            criterionId,
            score: 0.8,
            finding: `Fake finding for ${criterionId}.`,
          })),
        }
      : undefined;
  return {
...
export async function respond(prompt) {
  if (prompt.includes('You are judging the visual quality of a web application')) {
    return buildArtifact({
      stepId: 'ui-quality-judge',
      role: 'tester',
      taskKind: 'verification',
      mutationAllowed: false,
      outputSchemaId: UI_QUALITY_JUDGE_SCHEMA_ID,
    });
  }
