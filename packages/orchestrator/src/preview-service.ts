...
export class PreviewService {
  ...
  /** The project's live preview session, if one exists. `undefined` keeps the
   * project-wide lookup API; `null` addresses only the pre-#617 legacy root.
   * 'failing' does not count: it is already condemned, and a caller that
   * treated it as live would skip a boot the project is about to lose. */
    ...
    return records.find(
      (record) =>
        ...
        (environmentId === undefined ||
          (environmentId === null
            ? record.session.workspaceRef.environmentId === undefined
            : record.session.workspaceRef.environmentId === environmentId)) &&
        record.session.status !== 'failing',
    )?.session;
  }

  async renewForProject(projectId: string, environmentId?: string | null): Promise<boolean> {
