...
export function RetryPlanDialog({
  ...
  return (
    <Overlay
      ...
      eyebrow="REEXECUTAR STEP"
      title={retryPlan.step.stepId}
    >
      <form
        onSubmit={(event) => {
      ...
      >
        {retryPlan.plan.downstream.length > 0 ? (
          <div className="text-ink flex flex-col gap-2 text-[13px]">
            <p>
              Invalidar downstream reexecuta {retryPlan.plan.downstream.length} step(s) e gera novas
              revisões destes artifacts (o histórico anterior é preservado):
            </p>
